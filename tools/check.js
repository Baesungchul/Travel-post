...
const gates = [
  ...
  /* ☠️ 2026-09-08 2차 — 네이버는 title(EXTRA_SUBJECT)을 안 읽고 본문 앞부분으로 제목을
       만든다. 그래서 네이버에는 본문을 공유로 넘기지 않는다(글은 클립보드로).
       이 줄이 되돌아가면 "[공유] …" 제목이 그대로 되살아난다. */
  ['share.js',    'var NO_SHARE_BODY = { naver: true }', '네이버에는 본문을 공유로 안 넘긴다'],
  ['share.js',    "NO_SHARE_BODY[chId] ? ' '", '본문 대신 공백을 넘긴다 (빈 문자열은 안 받는 앱이 있다)'],
  ['share.js',    '제목 칸</b>에는 글의 <b>첫 줄', '제목을 직접 넣으라는 안내'],
