...
(function () {
  ...
  var CHANNELS = {
    ...
    /* ⭐ 2026-09-02 사용자 요청으로 켬 — 원래 "실측 전까지 ready:false" 였다(아래 이유).
       실측을 새로 한 건 아니지만, 공유 경로 자체(share.js run())는 naver·insta 와
       **완전히 같은 범용 코드**다 — 클립보드에 글 복사 + Capacitor Share.share({files:[...]})
       로 안드로이드 공유시트를 띄우는 것뿐, 채널마다 다른 API 를 타지 않는다.
       그래서 막연히 꺼 두기보다 켜서 실제로 써 보고, 안 되면 알려달라고 하는 쪽을 골랐다.
         · 티스토리/브런치: (PC 링크 모드에서) 웹 에디터가 붙여넣은 외부 이미지를
           재업로드하는지는 여전히 미확인 — 모바일 공유시트 경로는 이 문제와 무관하다.
         · 스레드/X: 안드로이드 공유시트가 사진 여러 장을 앱에 넘기는지 미확인 —
    ...
    threads: {
      ...
      sys: '당신은 짧은 소셜 게시글을 쓰는 한국어 카피라이터입니다. {카테고리} 방문을 스레드(Threads)용으로 ' +
           '3~4문장으로 압축하세요. 이 글은 {카테고리}에 대한 글입니다 — 다른 카테고리의 내용을 끌어오지 마세요.'
    },
    x: {
      ...
      sys: '당신은 짧은 소셜 게시글을 쓰는 한국어 카피라이터입니다. {카테고리} 방문을 X(트위터)용으로 ' +
           '280자를 넘지 않게 2~3문장으로 압축하세요. 이 글은 {카테고리}에 대한 글입니다 — 다른 카테고리의 내용을 끌어오지 마세요.'
    }
  };
  ...
  /* ── 채널 대표 아이콘 (사용자 요청 2026-09-02) ─────────────────
     ...
     tistory·threads·x 는 현장매니저에 없던 채널이라 같은 화풍(24x24, 둥근 배경 + 흰 글자)으로 새로 그렸다.
     ⚠️ 2026-09-02: 스레드·X 를 분리했다(전엔 '스레드 · X' 한 채널이었다) — 각자 다른
        모양(스레드는 @ 모양 고리, X 는 X자)으로 구분해야 목록에서 헷갈리지 않는다. */
  var CH_SVG = {
    ...
    threads: '<rect width="24" height="24" rx="6.5" fill="#000"/>'
           + '<circle cx="12" cy="12" r="5.4" fill="none" stroke="#fff" stroke-width="2.1"/>'
           + '<path d="M15.6 12c0 2.2-1.2 3.6-2.6 3.6-1 0-1.6-.7-1.6-1.7 0-1.6 1.6-2.6 3.6-2.6" '
           + 'fill="none" stroke="#fff" stroke-width="1.6" stroke-linecap="round"/>',
    x: '<rect width="24" height="24" rx="6.5" fill="#000"/>'
     + '<path d="M6.2 6.2L17.8 17.8M17.8 6.2L6.2 17.8" stroke="#fff" stroke-width="2.3" stroke-linecap="round"/>'
  };
  ...
  /* ── 추천 제목 3개를 글 맨 위에 (사용자 요청 2026-09-08, 현장매니저와 같은 처방) ──
     "글을 생성할 때 처음에 추천제목으로, 그 아래 추천하는 제목 3가지를 나열하고,
      그 아래부터 본문을 넣어줘. 사용자가 그중 하나를 선택해서 제목으로 붙여넣기 하면 되니까"

     ☠️ 왜 필요했나 — 네이버 블로그 앱은 공유로 받은 제목(EXTRA_SUBJECT)을 안 읽는다.
        그래서 제목 칸은 사장님이 손으로 채워야 한다. 글을 만들 때 같이 지어 두면
        고르기만 하면 되고, 본문과 한 번에 나오므로 **추가 요금이 없다**.
     ⚠️ 사용자 결정(2026-09-08): 이 블록은 복사할 때도 그대로 둔다. 블로그 본문에서 직접 지운다.
     ⚠️ 여기 규칙은 아래 generateTitles(「제목 후보」 버튼)와 같은 원칙이다 — 한쪽만 고치지 말 것.
        버튼은 "마음에 안 들면 다시 지어 줘" 용으로 남는다(값싼 모델로 따로 부른다).
     ☠️ 순위를 보장한다고 쓰지 말 것. 검색 규칙은 공개돼 있지도, 고정돼 있지도 않다. */
  var TITLE_BLOCK_GUIDE = [
    '[글의 맨 처음 — 추천 제목]',
    '본문을 쓰기 전에, 글 맨 위에 아래 형식 그대로 추천 제목 세 개를 먼저 적으세요.',
    '',
    '추천 제목',
    '1. (제목 후보)',
    '2. (제목 후보)',
    '3. (제목 후보)',
    '',
    '그 다음 빈 줄을 하나 두고 본문을 시작합니다.',
    '- 제목은 25~32자. 너무 길면 검색 결과에서 뒤가 잘립니다.',
    '- 지역명과 업종·메뉴 키워드를 앞쪽에 둡니다. 뒤로 갈수록 검색 가중치가 낮습니다.',
    '- 사람들이 실제로 검색창에 치는 말로 씁니다.',
    '- 상호명만 쓰지 않습니다. 상호를 아는 사람만 찾을 수 있어 새 방문자가 안 들어옵니다.',
    '- 세 개를 서로 다른 각도로: ①지역+업종 키워드 중심 ②본문의 구체적인 정보 중심 ③후기 성격',
    '- 과장·낚시·특수문자 남발은 하지 않습니다. 저품질로 걸리면 검색에서 아예 빠집니다.',
    '- 본문에 없는 내용을 제목에 넣지 않습니다.',
    '- 번호와 마침표(1. 2. 3.)만 붙이고 따옴표·설명은 붙이지 마세요.',
    '- 본문 안에서 이 세 제목을 다시 언급하지 마세요.'
  ].join('\n');
  ...
  async function generatePost(chId, extraMemo, place, model) {
    ...
    /* 추천 제목 블록은 블로그 글에만 — 인스타·스레드 캡션에 제목 후보가 있으면 지저분하다 */
    if (chId === 'naver' || chId === 'tistory') sys += '\n\n' + TITLE_BLOCK_GUIDE;
