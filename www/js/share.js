...
(function () {
  ...
  var STEPS = {
    ...
    naver: ['<b>1️⃣ 갤러리에 저장</b>을 누르세요 — 사진이 찍고쓰다 앨범에 담겨요',
            ...
            '본문에 글이 그대로 들어가요 (안 들어갔으면 본문 칸을 길게 눌러 <b>붙여넣기</b> — 글은 복사해 뒀어요)',
            /* ☠️ 2026-09-08 — 제목 칸은 네이버가 "[공유] + 본문 앞부분" 으로 채운다.
                 우리가 못 막으니(shareTextOnly 주석 ①②③) **옮겨 담는 법**을 알려 준다. */
            '본문 맨 위 <b>추천 제목</b> 중 하나를 길게 눌러 <b>복사</b> → <b>제목 칸의 글자를 모두 지우고</b> 거기에 <b>붙여넣기</b> 하세요 (＂[공유]…＂ 는 네이버가 붙인 것이라 지우면 돼요)',
  ...
  async function shareTextOnly(chId, text, place) {
    ...
    try {
      /* ═══ 네이버 제목에 대해 알아낸 것 — 다시 시도하지 말 것 (2026-09-08 실측) ═══
         ① title(EXTRA_SUBJECT)을 넘겨도 **네이버 블로그 앱은 안 읽는다.**
            Capacitor Share 는 title 을 EXTRA_SUBJECT 로 제대로 넣는다(플러그인 소스 확인).
            읽는 쪽이 무시하는 것이라 우리가 할 수 있는 게 없다.
         ② 네이버는 제목을 **"[공유] " + 넘겨받은 본문 앞부분** 으로 만든다.
            "[공유]" 는 네이버가 붙이는 말머리다 — 공유 시트를 쓰는 한 못 없앤다.
         ③ 그래서 본문을 안 넘겨 봤다(공백 한 칸). 결과는 제목 "[공유]" + 본문 텅 빔.
            사용자: "본문은 왜 비어있는거야??" → 되돌렸다.
         → 결론: 제목은 어차피 손을 대야 한다. 그렇다면 **본문이라도 자동으로 채워지는**
           쪽이 손이 덜 간다(사용자 결정 2026-09-08). 본문 전체를 그대로 넘긴다.
           대신 안내에 "추천 제목 하나를 복사해 제목 칸을 지우고 붙여넣으세요" 를 넣었다.
         ⚠️ title 은 계속 넘긴다. EXTRA_SUBJECT 를 읽는 앱에서는 제목이 제대로 들어간다. */
